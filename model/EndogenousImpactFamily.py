...
from model.EndogenousImpact import BasicEndogenousImpact,BasicEndogenousImpact_seq
...
class NaiveEndogenousImpact_seq(BasicEndogenousImpact_seq):
    """
    The class of naive endogenous impact functions sum_i phi_{kk_i}(t-t_i) for k = 1,...,C,
    which actually a simple endogenous impact with phi_{kk'}(t) = sum_{m} a_{kk'm} kernel_m(t)
    """
    ...
    def __init__(self, num_type: int, kernel, parameter_set: Dict):
        ...
        :param parameter_set: a dictionary containing parameters
            parameter_set = {'activation': value = names of activation layers ('identity', 'relu', 'softplus')}
        """
        super(NaiveEndogenousImpact_seq, self).__init__(num_type, kernel)
        activation = parameter_set['activation']
        if activation is None:
            self.endogenous_impact_type = "sum_m a_(kk'm) * kernel_m(t)"
            self.activation = 'identity'
    ...
    def intensity(self, sample_dict: Dict):
        ...
        :return:
            ...
            pHi: (batch_size, num_type) FloatTensor represents sum_{c, i in history} int_{start}^{stop} phi_cc_i(s)ds
        """
        event_times = sample_dict['tjs']     # (1, len(this seq))
        #history_time = sample_dict['tjs']  
        events = sample_dict['cjs']         
        #history = sample_dict['cjs']       
        endo_list = []
        endo_list.append(torch.tensor(0, device=events.device))
        endo_matrix = torch.zeros((event_times.shape[1],event_times.shape[1]),  device=events.device)
        
        
        for i in range(1,event_times.shape[1]):
            ui = events[:,i] # event type
            ti = event_times[0,i] 
            tj = event_times[0,:i]
            uj = events[:,:i]
            dts = (ti - tj).unsqueeze(0) # (batch_size=1, memory_size)
            gt = self.decay_kernel.values(dts)  
            #auiuj = Aest[uj_int, :, cluster_idx, ui_int] if (A_prop is None) else Aest[uj_int,:,ui_int]
            phi_c = 0
            resp_sum = torch.zeros_like(gt[:, :, 0]) 
            for m in range(self.num_base):
                A_cm = self.basis[m](ui)                        # (1, dim_embedding)
                #A_cm = A_cm.squeeze(1)                             
                A_cm = A_cm.gather(1, uj)                      # (1, history_size)
                A_cm = self.act(A_cm)
                A_cm = A_cm.unsqueeze(1)                            # (batch_size, 1, memory_size)
                resp = A_cm * gt[:, :, m]
                resp_sum += resp.squeeze(0) 
                phi_c += torch.bmm(A_cm, gt[:, :, m].unsqueeze(2))  # (batch_size, 1, 1)
            phi_c = phi_c[0, 0, 0]
            endo_list.append(phi_c)
            endo_matrix[i, :i] = resp_sum
        endo_tensor = torch.stack(endo_list).unsqueeze(0)
            
        return endo_tensor,endo_matrix
    ...
    def granger_causality(self, sample_dict: Dict):
        ...
        :return:
            A_all: (num_type, num_type, num_base) FloatTensor represents a_{cc'm} in phi_{cc'}(t)
        """
        all_types = sample_dict['cjs']  # (num_type, 1)
        A_all = 0
        for m in range(self.num_base):
            A_tmp = self.basis[m](all_types)  # (num_type, 1, num_type)
            A_tmp = self.act(torch.transpose(A_tmp, 1, 2))
