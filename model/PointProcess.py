...
class PointProcessModel(object):
    ...
    def fit_em(self, dataloader, optimizer, epochs: int, scheduler=None, sparsity: float=None, nonnegative=None,
            use_cuda: bool=False, validation_set=None):
        """
        Learn parameters of a generalized Hawkes process given observed sequences using EM algorithm (event level)
        :param dataloader: a pytorch batch-based data loader
        :param optimizer: the sgd optimization method defined by PyTorch
        :param epochs: the number of training epochs
        :param scheduler: the method adjusting the learning rate of SGD defined by PyTorch
...
class PointProcessModel_seq(object):
    """
    The class of generalized Hawkes process model
    contains most of necessary function.
    """
    ...
    def __init__(self, num_type, mu_dict, loss_type, use_cuda):
        ...
        exogenousIntensity = getattr(model.ExogenousIntensityFamily, mu_dict['model_name'])
        self.lambda_model = exogenousIntensity(num_type, mu_dict['parameter_set'])
        self.eps = float(np.finfo(np.float32).eps)   # to avoid NaN problem
        self.loss_type = loss_type
        if self.loss_type == 'mle':
            self.loss_function = MaxLogLike()
        elif self.loss_type == 'ls':
            ...
            for batch_idx, samples in enumerate(dataloader):
                ...
                if sparsity is not None:
                    for parameter in self.lambda_model.parameters():
                        reg += sparsity * torch.sum(torch.abs(parameter))
                loss_total = loss + reg
                minibatchloss_total += loss_total
                if (batch_idx+1) % 5 == 0:
                    minibatchloss_avg = minibatchloss_total/5
                    (minibatchloss_avg).backward()
                    optimizer.step()
                    ...
                    logger.info('Train Epoch: {} [{}/{} ({:.0f}%)]'.format(
                        epoch+1, (batch_idx+1) * ci.size(0), len(dataloader.dataset), 100. * batch_idx / len(dataloader)))
                    if sparsity is not None:
                        logger.info('Loss per sequence: {:.6f}, Regularizer: {:.6f} Time={:.2f}sec'.format(
                            loss.data, reg.data, time.time() - start))
                    else:
                        logger.info('Loss per sequence: {:.6f}, Regularizer: {:.6f} Time={:.2f}sec'.format(
                            minibatchloss_avg.data, 0, time.time() - start))
                    minibatchloss_total = 0

        if validation_set is not None:
            validation_loss = self.validation(validation_set, use_cuda)
            logger.info('After Epoch: {}, validation loss per sequence: {:.6f}.\n'.format(epoch, validation_loss))
            if validation_loss < best_loss:
                best_model = copy.deepcopy(self.lambda_model)
                best_loss = validation_loss
    ...
    def e_step(self, batch_dict):
        lambda_t, Lambda_t = self.lambda_model(batch_dict)
        mu_events,_ = self.lambda_model.exogenous_intensity(batch_dict)
        g_events, endo_mat, _ = self.lambda_model.endogenous_intensity(batch_dict)
        #print(len(g_events))
        return mu_events, g_events, endo_mat, lambda_t
    
    def m_step(self, mu_events, endo_mat, lambda_t, Lambda_t):
        non_zero_indices = torch.nonzero(endo_mat)
        neg_loglike = -(((mu_events+self.eps).log())*(mu_events/ lambda_t)).sum() - (((endo_mat[non_zero_indices]+self.eps).log())*(endo_mat[non_zero_indices]/lambda_t[non_zero_indices[:, 0]])).sum() + Lambda_t.sum()
        return neg_loglike
    
    def fit_sinkhorn_em(self):
        pass
    
    def fit_em(self, dataloader, optimizer, epochs: int, scheduler=None, sparsity: float=None, nonnegative=None,
            use_cuda: bool=False, validation_set=None):
        """
        Learn parameters of a generalized Hawkes process given observed sequences using EM algorithm
        :param dataloader: a pytorch batch-based data loader
        :param optimizer: the sgd optimization method defined by PyTorch
        :param epochs: the number of training epochs
        :param scheduler: the method adjusting the learning rate of SGD defined by PyTorch
        ...
        for epoch in range(epochs):
            ...
            for batch_idx, samples in enumerate(dataloader):
                ...
                lambda_t, Lambda_t = self.lambda_model(batch_dict)
                    ###########################################       
                    # E-step:    compute_responsibilities
                    ########################################### 
                mu_events, g_events, endo_mat, lambda_t = self.e_step(batch_dict)   
                            
                    ###########################################       
                    # M-step:    Minimize NLL
                    ########################################### 
                loss = self.m_step(mu_events, endo_mat, lambda_t, Lambda_t)
